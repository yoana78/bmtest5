...
// 관리자(CMS)가 편집한 문구가 있으면 그것을, 없으면 코드 기본값을 그대로 보여준다.
// useCopy()/copy-en.json은 이 페이지의 다른 고정 UI 문구(버튼 aria-label 등)에만 계속 쓰인다.
const preLine = { whiteSpace: 'pre-line' }
...
function Opening({ onFilm }) {
  const { tr } = useCopy()
  const { txt } = usePageContent('landing')
  ...
  const scenes = [
    { file:'hero-beginning', label:'SINCE 1995', content:<><p className="hero-year">1995</p><p className="hero-year-note" style={preLine}>{txt('hero1Note')}</p></> },
    { file:'research', label:'RESPECT FOR PET LIFE', content:<p className="hero-slogan" style={preLine}>{txt('hero2Slogan')}</p> },
    { file:'hero-making', label:'PET FOOD & SUPPLIES · OEM / ODM', content:<><p className="hero-company">{txt('hero3Name')}</p><p className="hero-company-en">BOOMYUNG</p></> },
    { file:'hero-together', label:'BETTER EVERYDAY, TOGETHER', content:<><p className="hero-sub-slogan" style={preLine}>{txt('hero4Sub')}</p><div className="intro-actions"><button onClick={onFilm}>{txt('hero4FilmButton')} <span>↗</span></button><Link to="/#business">{txt('hero4BizLink')} <span>↓</span></Link></div></> },
  ]
...
function LensScene() {
  ...
  return <section className="cinema-lens" id="story" data-scroll-scene="lens" aria-label={tr("제품에서 일상으로")}>
    <div className="cinema-sticky lens-stage">
      ...
      <h2 className="lens-statement" style={preLine}>{txt('lensStatement')}</h2>
      <span className="lens-footnote">{txt('lensFootnote')}</span>
    </div>
  </section>
}

const getChapters = txt => [
  {
    key:'MAKE', number:'01', theme:'dark', title:txt('makeTitle'),
    description:txt('makeDesc'),
    image:`${MEDIA}intro-make.webp`,
    cards:[
      { label:'OEM / ODM', title:txt('makeCard1Title'), body:txt('makeCard1Body'), image:`${MEDIA}research.webp`, video:`${MEDIA}research-card-web.mp4`, to:'/contact?type=oem', cta:txt('makeCard1Cta') },
      { label:'PET FOOD & SUPPLIES', title:txt('makeCard2Title'), body:txt('makeCard2Body'), image:'/assets/renewal/brand-dayspo.jpg', to:'/products', cta:txt('makeCard2Cta') },
    ],
  },
  {
    key:'CONNECT', number:'02', theme:'light', title:txt('connectTitle'),
    description:txt('connectDesc'),
    image:`${MEDIA}intro-connect.webp`,
    cards:[
      { label:'DOMESTIC DISTRIBUTION', title:txt('connectCard1Title'), body:txt('connectCard1Body'), image:'/assets/renewal/logistics-poster.jpg', video:'/assets/renewal/logistics.mp4', to:'/contact?type=domestic', cta:txt('connectCard1Cta') },
      { label:'OUR BRANDS', title:txt('connectCard2Title'), body:txt('connectCard2Body'), image:'/assets/renewal/brand-bellbird.jpg', to:'/products', cta:txt('connectCard2Cta') },
    ],
  },
  {
    key:'BEYOND', number:'03', theme:'blue', title:txt('beyondTitle'),
    description:txt('beyondDesc'),
    image:`${MEDIA}intro-beyond.webp`,
    cards:[
      { label:'GLOBAL BUSINESS', title:txt('beyondCard1Title'), body:txt('beyondCard1Body'), image:`${MEDIA}port-motion.webp`, video:`${MEDIA}port-motion-web.mp4`, to:'/contact?type=export', cta:txt('beyondCard1Cta') },
      { label:'GROW TOGETHER', title:txt('beyondCard2Title'), body:txt('beyondCard2Body'), image:`${MEDIA}hero.webp`, to:'/contact', cta:txt('beyondCard2Cta') },
    ],
  },
]
...
function BusinessRail() {
  const { tr } = useCopy()
  const { txt } = usePageContent('landing')
  const chapters = getChapters(txt)
    ...
    <div className="cinema-sticky rail-window">
      ...
      <div className="cinema-track" onFocusCapture={focusPanel}>
        {chapters.map(c=><div className={`cinema-chapter chapter-${c.theme}`} key={c.key}>
          <article className="chapter-intro"><span className="chapter-en">{c.key}</span><h2 style={preLine}>{c.title}</h2><p>{c.description}</p><img className="chapter-intro-image" src={c.image} alt="" loading="lazy"/></article>
          {c.cards.map(card=><article className="cinema-card" key={card.title}><CardMedia image={card.image} video={card.video} label={card.label}/><Link className="business-card-link" to={card.to}><span className="card-arrow" aria-hidden="true">↗</span><div className="cinema-card-copy"><span>{card.label}</span><h3>{card.title}</h3><p>{card.body}</p><b>{card.cta} <span>→</span></b></div></Link></article>)}
        </div>)}
      </div>
...
function BrandClosing() {
  const { tr, en } = useCopy()
  const { txt, img } = usePageContent('landing')

  const { brands } = useData()
  return <><div className="cinema-brands site-d-brand-wrapper"><SiteDBrandCarousel brands={brands} en={en} subtitle={txt('brandSubtitle')}/></div><section className="cinema-contact"><div className="cinema-contact-title"><h2>CONTACT</h2><div><p>{txt('contactCtaBody')}</p><Link to="/contact">{txt('contactCtaButton')} <span>↗</span></Link></div></div><div className="closing-landscape"><img src={img('contactCtaImage') || `${MEDIA}companionship.webp`} alt={tr("햇살이 드는 테라스에서 반려견과 교감하는 사람")} loading="lazy"/><span style={preLine}>{txt('contactCtaCaption')}</span></div></section></>
}
