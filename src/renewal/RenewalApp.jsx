...
function Footer() {
  ...
  return <footer className="footer"><div className="footer-top"><Logo/><div className="footer-links"><Link to="/products">{tr("제품 카테고리")} <Arrow/></Link><Link to="/contact">{tr("문의하기")} <Arrow/></Link></div></div><div className="footer-bottom"><div><strong>{tr("(주)부명")}</strong><p>{tr("경기도 구리시 건원대로34번길 19, 306호")}<br/>{tr("사업자등록번호 132-81-49973")}</p></div><div><a href="tel:0315538003">{tr("TEL")} 031.553.8003</a><br/><a href="mailto:help@petsb2b.co.kr">{tr("E-Mail")} help@petsb2b.co.kr</a></div><div><button onClick={() => setPrivacy(true)}>{tr("개인정보처리방침")}</button> <button onClick={() => setTerms(true)}>{tr("이용안내")}</button><p>© {new Date().getFullYear()} BOOMYUNG. All rights reserved.</p></div></div>{privacy && <Modal title={tr("개인정보처리방침")} onClose={() => setPrivacy(false)}><div className="privacy-text">{en ? privacyPolicyEn : privacyPolicyKo}</div></Modal>}{terms && <Modal title={tr("이용안내")} onClose={() => setTerms(false)}><div className="privacy-text">{en ? termsOfServiceEn : termsOfServiceKo}</div></Modal>}</footer>
}
function Modal({ title, onClose, children, className = 'modal' }) {
  const { tr } = useCopy()

  const ref = useRef(null)
  useEffect(() => { const el = ref.current; const previous = document.activeElement; el.showModal(); return () => { el.close(); previous?.focus() } }, [])
  return <dialog ref={ref} className={className} onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose() }}><button className="modal-close" onClick={onClose} aria-label={tr("닫기")}>×</button>{title && <h2>{title}</h2>}{children}</dialog>
}
const NUTRITION_LABELS = [['protein', '조단백'], ['fat', '조지방'], ['fiber', '조섬유'], ['moisture', '수분']]
// Product detail: image stage on the left, spec sheet on the right. The purchase
// button only renders when the admin has saved an http(s) link for the product.
function ProductModal({ product: p, brand, onClose }) {
  const { tr, en } = useCopy()
  const name = en ? (p.nameEn || p.nameKo) : p.nameKo
  const features = (en && p.featuresEn?.length ? p.featuresEn : p.features) || []
  const origin = en ? (p.originEn || p.origin) : p.origin
  const shelfLife = en ? (p.shelfLifeEn || p.shelfLife) : p.shelfLife
  const ingredients = en ? (p.ingredientsEn || p.ingredients) : p.ingredients
  const nutrition = NUTRITION_LABELS.filter(([key]) => String(p.nutrition?.[key] ?? '').trim())
  const buyUrl = String(p.purchaseUrl || '').trim()
  // admin saves detail-page images as `infoImages` (array of URLs)
  const detailImages = (p.infoImages?.length ? p.infoImages : p.detailImages || []).filter(Boolean)
  return <Modal className="modal product-modal" onClose={onClose}><div className="pd"><div className="pd-media"><img src={p.image} alt={name}/></div><div className="pd-body">{brand && <span className="pd-brand">{en ? (brand.nameEn || brand.nameKo) : brand.nameKo}</span>}<h2>{name}</h2><div className="pd-meta">{p.code && <span><strong>{tr("바코드")}</strong> {p.code}</span>}{p.spec &&<span><strong>{tr("규격")}</strong> {p.spec}</span>}{origin && <span><strong>{tr("원산지")}</strong> {tr(origin)}</span>}</div>{shelfLife && <p className="pd-shelf"><strong>{tr("유통기한")}</strong> {shelfLife}</p>}{features.length > 0 && <ul className="pd-features">{features.map((f,i)=><li key={i}>{f}</li>)}</ul>}{nutrition.length > 0 && <div className="pd-nutrition">{nutrition.map(([key,label])=><div key={key}><span>{tr(label)}</span><b>{p.nutrition[key]}</b></div>)}</div>}{ingredients && <div className="pd-ingredients"><strong>{tr("원료")}</strong><p>{ingredients}</p></div>}<div className="pd-actions">{/^https?:\/\//i.test(buyUrl) && <a className="pill dark" href={buyUrl} target="_blank" rel="noopener noreferrer">{tr("바로 구매하기")} <Arrow/></a>}<Link className="pill" to={`/contact?product=${encodeURIComponent(name)}`}>{tr("이 제품 문의하기")} <Arrow/></Link></div>{detailImages.length > 0 && <div className="pd-detail-images">{detailImages.map(src=><img key={src} src={src} alt={name}/>)}</div>}</div></div></Modal>
}
